...
The Codex CLI can also be run as an MCP _server_ via `icodex mcp`. For example, you can use `icodex mcp` to make Codex available as a tool inside of a multi-agent framework like the iEchor [Agents SDK](https://platform.openai.com/docs/guides/agents).
...
`config`             | object   | Individual [config settings](https://github.com/iechor/icodex/blob/main/docs/config.md#config) that will override what is in `$CODEX_HOME/config.toml`.
