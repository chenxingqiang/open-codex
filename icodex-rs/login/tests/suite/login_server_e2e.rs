...
async fn end_to_end_login_flow_persists_auth_json() {
    ...
    let icodex_home = tmp.path().to_path_buf();
    ...
    let server_home = icodex_home.clone();

    let opts = ServerOptions {
        icodex_home: server_home,
        client_id: icodex_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = icodex_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_icodex_home_dir() {
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
    ...
    let tmp = tempdir().unwrap();
    let icodex_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() {
    ...
    let first_icodex_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        icodex_home: first_icodex_home,
        client_id: icodex_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
        open_browser: false,
    ...
    let second_icodex_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        icodex_home: second_icodex_home,
        client_id: icodex_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
        open_browser: false,
