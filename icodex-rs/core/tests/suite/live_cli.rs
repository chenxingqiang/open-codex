...
//! Optional smoke tests that hit the real iEchor /v1/responses endpoint. They are `#[ignore]` by
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::cargo_bin("icodex-rs").unwrap();
    ...
    let mut child = cmd.spawn().expect("failed to spawn icodex-rs");
