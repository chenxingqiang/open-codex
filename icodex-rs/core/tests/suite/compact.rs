...
async fn summarize_context_three_requests_and_instructions() {
    ...
    icodex.submit(Op::Compact).await.unwrap();
    wait_for_event(&icodex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    ...
    icodex.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&icodex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn auto_compact_stops_after_failed_attempt() {
    ...
    let error_event = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::Error(_))).await;
...
async fn auto_compact_allows_multiple_attempts_when_interleaved_with_other_turn_events() {
    ...
    loop {
        let event = icodex.next_event().await.unwrap();
