...
use icodex_core::protocol::ExitedReviewModeEvent;
use icodex_core::protocol::InputItem;
use icodex_core::protocol::Op;
use icodex_core::protocol::ReviewCodeLocation;
use icodex_core::protocol::ReviewFinding;
use icodex_core::protocol::ReviewLineRange;
use icodex_core::protocol::ReviewOutputEvent;
use icodex_core::protocol::ReviewRequest;
...
async fn review_op_emits_lifecycle_and_review_output() {
    ...
    let icodex = new_conversation_for_server(&server, &icodex_home, |_| {}).await;
    ...
    let _entered = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
...
async fn review_does_not_emit_agent_message_on_structured_output() {
    ...
    loop {
        let ev = timeout(Duration::from_secs(5), icodex.next_event())
            .await
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let icodex = new_conversation_for_server(&server, &icodex_home, |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&icodex, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = icodex_home.path().join("resume.jsonl");
    {
        ...
        let user = icodex_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = icodex_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let icodex =
        resume_conversation_for_server(&server, &icodex_home, session_file.clone(), |_| {}).await;
