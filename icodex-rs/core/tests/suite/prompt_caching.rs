...
use icodex_core::model_family::find_family_for_model;
...
use icodex_core::shell::default_user_shell;
...
async fn icodex_mini_latest_tools() {
    ...
    config.model = "icodex-mini-latest".to_string();
    config.model_family = find_family_for_model("icodex-mini-latest").unwrap();
...
async fn prompt_tools_are_consistent_across_requests() {
    ...
    // with the iEchor schema, so we just verify the tool presence here
