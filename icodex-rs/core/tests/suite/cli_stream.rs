...
use icodex_core::protocol::GitInfo;
...
/// This test:
/// 1. Sets up a mock server that simulates iEchor's chat completions API
/// 2. Configures icodex to use this mock server via a custom provider
...
/// received by a mock iEchor Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
/// 2. Configures icodex to read from this fixture via CODEX_RS_SSE_FIXTURE env var
...
async fn integration_creates_and_checks_session_file() {
    ...
    // 4. Run the icodex CLI through cargo (ensures the right bin is built) and invoke `exec`,
    ...
    assert!(output2.status.success(), "resume icodex-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = icodex_core::git_info::collect_git_info(&git_repo).await;
