...
use icodex_core::protocol::ConversationPathResponseEvent;
use icodex_core::protocol::EventMsg;
use icodex_core::protocol::InputItem;
use icodex_core::protocol::Op;
use icodex_core::protocol::RolloutItem;
use icodex_core::protocol::RolloutLine;
...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    for text in ["first", "second", "third"] {
        icodex
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&icodex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    }

    // Request history from the base conversation to obtain rollout path.
    icodex.submit(Op::GetPath).await.unwrap();
    let base_history =
        wait_for_event(&icodex, |ev| matches!(ev, EventMsg::ConversationPath(_))).await;
    ...
    icodex_fork1.submit(Op::GetPath).await.unwrap();
    let fork1_history = wait_for_event(&icodex_fork1, |ev| {
    ...
    icodex_fork2.submit(Op::GetPath).await.unwrap();
    let fork2_history = wait_for_event(&icodex_fork2, |ev| {
        matches!(ev, EventMsg::ConversationPath(_))
    })
    .await;
