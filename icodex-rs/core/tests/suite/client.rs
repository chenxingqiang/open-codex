...
use icodex_core::ContentItem;
use icodex_core::ConversationManager;
use icodex_core::LocalShellAction;
use icodex_core::LocalShellExecAction;
use icodex_core::LocalShellStatus;
use icodex_core::ModelClient;
use icodex_core::ModelProviderInfo;
use icodex_core::NewConversation;
use icodex_core::Prompt;
use icodex_core::ReasoningItemContent;
use icodex_core::ResponseEvent;
use icodex_core::ResponseItem;
use icodex_core::WireApi;
use icodex_core::built_in_model_providers;
use icodex_core::protocol::EventMsg;
use icodex_core::protocol::InputItem;
use icodex_core::protocol::Op;
use icodex_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use icodex_protocol::mcp_protocol::ConversationId;
use icodex_protocol::models::ReasoningItemReasoningSummary;
use icodex_protocol::models::WebSearchAction;
...
/// Writes an `auth.json` into the provided `icodex_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        icodex_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = icodex_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![icodex_protocol::models::ContentItem::InputText {
    ...
    let prior_system = icodex_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![icodex_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = icodex_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let icodex_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&icodex_home);
    ...
    let auth_manager =
        icodex_core::AuthManager::from_auth_for_testing(CodexAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: icodex,
        session_configured,
        ..
    ...
    icodex
        ...
        .await
        .unwrap();
    wait_for_event(&icodex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "icodex_cli_rs");
    assert_eq!(
...
async fn includes_base_instructions_override_in_request() {
    ...
    let conversation_manager =
        ConversationManager::with_auth(CodexAuth::from_api_key("Test API Key"));
    let icodex = conversation_manager
        .new_conversation(config)
        .await
        .expect("create new conversation")
        .conversation;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/api/icodex", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    ...
    let mut config = load_default_config_for_test(&icodex_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_icodex_auth());
    let NewConversation {
        conversation: icodex,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match CodexAuth::from_icodex_home(icodex_home.path()) {
        Ok(Some(auth)) => icodex_core::AuthManager::from_auth_for_testing(auth),
        Ok(None) => panic!("No CodexAuth found in icodex_home"),
...
fn create_dummy_icodex_auth() -> CodexAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    ...
    // Init session with isolated icodex home.
    let icodex_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&icodex_home);
    config.model_provider = model_provider;
