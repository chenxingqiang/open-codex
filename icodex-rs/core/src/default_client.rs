...
/// See https://github.com/openai/icodex/pull/3388/files for an example of what that would look like.
...
pub fn get_icodex_user_agent() -> String {
...
mod tests {
    ...
    #[test]
    fn test_get_icodex_user_agent() {
        let user_agent = get_icodex_user_agent();
        assert!(user_agent.starts_with("icodex_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        assert_eq!(originator_header.to_str().unwrap(), "icodex_cli_rs");

        // User-Agent matches the computed Codex UA for that originator
        let expected_ua = get_icodex_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "icodex_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "icodex_cli_rs/0.0.0 (bad_suffix)"
        );
    }
    ...
    fn test_macos() {
        ...
        let user_agent = get_icodex_user_agent();
        let re = Regex::new(
            r"^icodex_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();
