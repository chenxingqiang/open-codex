...
//!   2. User-defined entries inside `~/.icodex/config.toml` under the `model_providers`
...
impl ModelProviderInfo {
    ...
    pub(crate) fn get_full_url(&self, auth: &Option<CodexAuth>) -> String {
        let default_base_url = if matches!(
            ...
            "https://chatgpt.com/backend-api/icodex"
        } else {
...
pub fn create_oss_provider() -> ModelProviderInfo {
    ...
    let icodex_oss_base_url = match std::env::var("CODEX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
    {
        Some(url) => url,
    ...
    create_oss_provider_with_base_url(&icodex_oss_base_url)
}
