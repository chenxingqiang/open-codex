...
//!
//! The history is stored at `~/.icodex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.icodex`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.icodex_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.icodex/history.jsonl` and ensure the parent directory exists.
