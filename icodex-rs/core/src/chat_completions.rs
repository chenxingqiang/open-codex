...
use icodex_protocol::models::ReasoningItemContent;
            ...
            match Pin::new(&mut this.inner).poll_next(cx) {
                ...
                Poll::Ready(Some(Ok(ResponseEvent::OutputItemDone(item)))) => {
                    ...
                    let is_assistant_message = matches!(
                        &item,
                        icodex_protocol::models::ResponseItem::Message { role, .. } if role == "assistant"
                    );
                        ...
                        match this.mode {
                            AggregateMode::AggregatedOnly => {
                                ...
                                if this.cumulative.is_empty()
                                    && let icodex_protocol::models::ResponseItem::Message {
                                        content,
                                        ..
                                    } = &item
                                    && let Some(text) = content.iter().find_map(|c| match c {
                                        icodex_protocol::models::ContentItem::OutputText {
                                            text,
                                        } => Some(text),
                                        _ => None,
                                    })
                ...
                Poll::Ready(Some(Ok(ResponseEvent::Completed {
                        ...
                        && matches!(this.mode, AggregateMode::AggregatedOnly)
                    {
                        let aggregated_reasoning =
                            icodex_protocol::models::ResponseItem::Reasoning {
                                id: String::new(),
                                summary: Vec::new(),
                                content: Some(vec![
                                    icodex_protocol::models::ReasoningItemContent::ReasoningText {
                    ...
                    if !this.cumulative.is_empty() {
                        let aggregated_message = icodex_protocol::models::ResponseItem::Message {
                            id: None,
                            role: "assistant".to_string(),
