...
use anyhow::Context;
use icodex_protocol::config_types::ReasoningEffort;
use icodex_protocol::config_types::ReasoningSummary;
use icodex_protocol::config_types::SandboxMode;
use icodex_protocol::config_types::Verbosity;
use icodex_protocol::mcp_protocol::Tools;
...
pub const GPT_5_CODEX_MEDIUM_MODEL: &str = "gpt-5-icodex";
...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.icodex/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Directory containing all Codex state (defaults to `~/.icodex` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub icodex_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.icodex/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `icodex-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `icodex-linux-sandbox`.
    pub icodex_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    ...
    pub fn load_with_cli_overrides(
        ...
        // Resolve the directory that stores Codex state (e.g. ~/.icodex or the
        // value of $CODEX_HOME) so we can embed it into the resulting
        // `Config` instance.
        let icodex_home = find_icodex_home()?;

        // Step 1: parse `config.toml` into a generic JSON value.
        let mut root_value = load_config_as_toml(&icodex_home)?;
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, icodex_home)
    }
}
...
pub fn load_config_as_toml(icodex_home: &Path) -> std::io::Result<TomlValue> {
    let config_path = icodex_home.join(CONFIG_TOML_FILE);
...
pub fn load_global_mcp_servers(
    ...
    let root_value = load_config_as_toml(icodex_home)?;
...
pub fn write_global_mcp_servers(
    ...
    std::fs::create_dir_all(icodex_home)?;
    let tmp_file = NamedTempFile::new_in(icodex_home)?;
...
/// Use with caution.
pub fn set_project_trusted(icodex_home: &Path, project_path: &Path) -> anyhow::Result<()> {
    let config_path = icodex_home.join(CONFIG_TOML_FILE);
...
pub async fn persist_model_selection(
    ...
    tokio::fs::create_dir_all(icodex_home)
        .await
        .with_context(|| {
            format!(
...
/// Base config deserialized from ~/.icodex/config.toml.
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let user_instructions = Self::load_instructions(Some(&icodex_home));
    ...
    fn load_instructions(icodex_dir: Option<&Path>) -> Option<String> {
        let mut p = match icodex_dir {
...
///   directory exists.
pub fn find_icodex_home() -> std::io::Result<PathBuf> {
...
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.icodex_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        let icodex_home = TempDir::new()?;

        let servers = load_global_mcp_servers(icodex_home.path())?;
    ...
    fn write_global_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        write_global_mcp_servers(icodex_home.path(), &servers)?;

        let loaded = load_global_mcp_servers(icodex_home.path())?;
        ...
        write_global_mcp_servers(icodex_home.path(), &empty)?;
        let loaded = load_global_mcp_servers(icodex_home.path())?;
    ...
    async fn persist_model_selection_updates_defaults() -> anyhow::Result<()> {
        ...
        let serialized =
            tokio::fs::read_to_string(icodex_home.path().join(CONFIG_TOML_FILE)).await?;
        let parsed: ConfigToml = toml::from_str(&serialized)?;

        assert_eq!(parsed.model.as_deref(), Some("gpt-5-icodex"));
    ...
    async fn persist_model_selection_overwrites_existing_model() -> anyhow::Result<()> {
        let icodex_home = TempDir::new()?;
        let config_path = icodex_home.path().join(CONFIG_TOML_FILE);
...
"#,
    ...
    async fn persist_model_selection_updates_profile() -> anyhow::Result<()> {
        ...
        assert_eq!(profile.model.as_deref(), Some("gpt-5-icodex"));
        assert_eq!(
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn icodex_home(&self) -> PathBuf {
            self.icodex_home.path().to_path_buf()
        }
    }
...
"#;
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            icodex_home: icodex_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                icodex_home: fixture.icodex_home(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                icodex_linux_sandbox_exe: None,
...
projects = { "/Users/mbolin/code/icodex4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/icodex3" = { trust_level = "trusted" } }
model = "foo""#;
        let mut doc = initial.parse::<DocumentMut>()?;

        // Approve a new directory
        let new_project = Path::new("/Users/mbolin/code/icodex2");
...
[projects."/Users/mbolin/code/icodex4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/icodex3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/icodex2"]
trust_level = "trusted"
"#;
        assert_eq!(contents, expected);
