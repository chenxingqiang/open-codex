...
pub async fn persist_overrides(
    ...
    persist_overrides_with_behavior(icodex_home, profile, &with_options, NoneBehavior::Skip).await
}
...
pub async fn persist_non_null_overrides(
    ...
    persist_overrides_with_behavior(icodex_home, profile, overrides, NoneBehavior::Skip).await
}
...
pub async fn persist_overrides_and_clear_if_none(
    ...
    persist_overrides_with_behavior(icodex_home, profile, overrides, NoneBehavior::Remove).await
}
...
async fn persist_overrides_with_behavior(
    ...
    let mut doc = match read_result {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            ...
            tokio::fs::create_dir_all(icodex_home).await?;
            DocumentMut::new()
        }
...
mod tests {
    ...
    async fn set_default_model_and_effort_top_level_when_no_profile() {
        ...
        let contents = read_config(icodex_home).await;
...
"#;
    ...
    async fn set_defaults_update_profile_when_profile_set() {
        ...
        let seed = "profile = \"o3\"\n";
        tokio::fs::write(icodex_home.join(CONFIG_TOML_FILE), seed)
            .await
            .expect("seed write");
...
"#;
    ...
    async fn set_defaults_update_when_profile_override_supplied() {
        ...
        tokio::fs::write(icodex_home.join(CONFIG_TOML_FILE), "")
            .await
            .expect("seed write");
...
"#;
    ...
    async fn persist_overrides_replaces_scalar_with_table() {
        ...
        persist_overrides(icodex_home, None, &[(&["foo", "bar", "baz"], "ok")])
            .await
            .expect("persist");
...
"#;
    ...
    async fn persist_overrides_errors_on_parse_failure() {
        ...
        tokio::fs::write(icodex_home.join(CONFIG_TOML_FILE), invalid)
            .await
            .expect("seed write");

        // Attempting to persist should return an error and must not clobber the file.
        let res = persist_overrides(icodex_home, None, &[(&["x"], "y")]).await;
    ...
    async fn changing_only_model_preserves_existing_effort_top_level() {
        ...
        persist_overrides(icodex_home, None, &[(&[CONFIG_KEY_MODEL], "o3")])
            .await
            .expect("persist");
...
"#;
    ...
    async fn changing_only_effort_preserves_existing_model_top_level() {
        ...
        persist_overrides(icodex_home, None, &[(&[CONFIG_KEY_EFFORT], "high")])
            .await
            .expect("persist");
...
"#;
        ...
        persist_overrides(icodex_home, None, &[(&[CONFIG_KEY_MODEL], "o4-mini")])
            .await
            .expect("persist");
...
"#;
    ...
    async fn persist_non_null_noop_when_all_none() {
        ...
        assert!(!icodex_home.join(CONFIG_TOML_FILE).exists());
    }
...
"#;
    ...
    async fn persist_clear_none_noop_when_file_missing() {
        ...
        persist_overrides_and_clear_if_none(icodex_home, None, &[(&[CONFIG_KEY_MODEL], None)])
            .await
            .expect("persist");
    ...
    async fn read_config(icodex_home: &Path) -> String {
        let p = icodex_home.join(CONFIG_TOML_FILE);
