...
use chrono::DateTime;
use chrono::Utc;
use icodex_core::ConversationItem;
use icodex_core::ConversationsPage;
use icodex_core::Cursor;
...
pub async fn run_resume_picker(tui: &mut Tui, icodex_home: &Path) -> Result<ResumeSelection> {
    let alt = AltScreenGuard::enter(tui);
    let mut state = PickerState::new(icodex_home.to_path_buf(), alt.tui.frame_requester());
...
impl PickerState {
    fn new(icodex_home: PathBuf, requester: FrameRequester) -> Self {
        Self {
            icodex_home,
            requester,
            pagination: Pagination {
    ...
    async fn load_page(&mut self, anchor: Option<&Cursor>) -> Result<()> {
        let page = RolloutRecorder::list_conversations(&self.icodex_home, PAGE_SIZE, anchor).await?;
