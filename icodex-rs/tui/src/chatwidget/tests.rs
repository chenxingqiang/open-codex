...
use icodex_core::plan_tool::PlanItemArg;
use icodex_core::plan_tool::StepStatus;
use icodex_core::plan_tool::UpdatePlanArgs;
...
use icodex_core::protocol::PatchApplyEndEvent;
use icodex_core::protocol::StreamErrorEvent;
use icodex_core::protocol::TaskCompleteEvent;
use icodex_core::protocol::TaskStartedEvent;
...
fn final_answer_without_newline_is_flushed_immediately() {
    ...
    chat.handle_icodex_event(Event {
        id: "sub-a".into(),
        msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent {
            delta: "Hi! How can I help with icodex-rs or anything else today?".into(),
        }),
    });
    ...
    chat.handle_icodex_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: "Hi! How can I help with icodex-rs or anything else today?".into(),
        }),
    });
    ...
    assert!(
        ...
        "expected 'icodex' header to be emitted",
    );
    let found_final = cells.iter().any(|lines| {
        let s = lines
            ...
            .map(|sp| sp.content.clone())
            .collect::<String>();
        s.contains("Hi! How can I help with icodex-rs or anything else today?")
    });
    assert!(
        found_final,
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = icodex_core::protocol::SessionConfiguredEvent {
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<ParsedCommand> = icodex_core::parse_command::parse_command(&command)
        .into_iter()
        .map(Into::into)
        .collect();
    chat.handle_icodex_event(Event {
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_icodex_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(icodex_core::protocol::TurnAbortedEvent {
                ...
                if let Some(payload) = v.get("payload") {
                    ...
                    let ev = match ev {
                        Event {
                            ...
                            let parsed_cmd = icodex_core::parse_command::parse_command(&e.command);
                            Event {
                                id: ev.id,
                                msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert!(matches!(
                decision,
                icodex_core::protocol::ReviewDecision::Approved
            ));
            found = true;
            break;
        }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto icodex_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to icodex channel");
    match forwarded {
...
fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    chat.handle_icodex_event(Event {
        ...
        msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
            ...
            cwd: std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            parsed_cmd: vec![
                icodex_core::parse_command::ParsedCommand::Search {
                    query: Some("Change Approved".into()),
                    path: None,
                    cmd: "rg \"Change Approved\"".into(),
                }
                .into(),
                icodex_core::parse_command::ParsedCommand::Read {
