...
use base64::Engine;
use icodex_ansi_escape::ansi_escape_line;
...
impl HistoryCell for AgentMessageCell {
    ...
    fn transcript_lines(&self) -> Vec<Line<'static>> {
        ...
        if self.is_first_line {
            out.push("icodex".magenta().bold().into());
        }
...
pub(crate) fn new_status_output(
    ...
    let auth_file = get_auth_file(&config.icodex_home);
        ...
        && let Some(tokens) = auth.tokens.clone()
    {
        ...
        match auth.openai_api_key.as_deref() {
            Some(key) if !key.is_empty() => {
                lines.push("  • Using API key. Run icodex login to use ChatGPT plan".into());
            }
            _ => {
                let plan_text = info
...
mod tests {
    ...
    fn reasoning_summary_block_splits_header_and_summary_when_present() {
        ...
        assert_eq!(
            summary_lines,
            vec!["icodex", "Thinking", "We should fix the bug next."]
        )
    }
}
