...
// The standalone `icodex-tui` binary prints a short help message before the
// alternate‑screen mode starts; that file opts‑out locally via `allow`.
#![deny(clippy::print_stdout, clippy::print_stderr)]
#![deny(clippy::disallowed_methods)]
use app::App;
use icodex_core::AuthManager;
use icodex_core::BUILT_IN_OSS_MODEL_PROVIDER_ID;
use icodex_core::CodexAuth;
...
use icodex_core::config::GPT_5_CODEX_MEDIUM_MODEL;
use icodex_core::config::find_icodex_home;
use icodex_core::config::load_config_as_toml_with_cli_overrides;
use icodex_core::config::persist_model_selection;
use icodex_core::find_conversation_path_by_id_str;
use icodex_core::protocol::AskForApproval;
use icodex_core::protocol::SandboxPolicy;
use icodex_ollama::DEFAULT_OSS_MODEL;
use icodex_protocol::config_types::SandboxMode;
...
use crate::tui::Tui;
pub use cli::Cli;
use icodex_core::internal_storage::InternalStorage;
...
pub async fn run_main(
...
) -> std::io::Result<icodex_core::protocol::TokenUsage> {
    ...
    let overrides_cli = icodex_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let config_toml = {
        let icodex_home = match find_icodex_home() {
            Ok(icodex_home) => icodex_home,
            Err(err) => {
                eprintln!("Error finding icodex home: {err}");
                std::process::exit(1);
            }
        };

        match load_config_as_toml_with_cli_overrides(&icodex_home, cli_kv_overrides) {
    ...
    let internal_storage = InternalStorage::load(&config.icodex_home);

    let log_dir = icodex_core::config::log_dir(&config)?;
    ...
    let log_file = log_file_opts.open(log_dir.join("icodex-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for icodex crates.
    let env_filter = || {
        EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("icodex_core=info,icodex_tui=info"))
    };
    ...
    if cli.oss {
        icodex_ollama::ensure_oss_ready(&config)
            .await
...
async fn run_ratatui_app(
...
) -> color_eyre::Result<icodex_core::protocol::TokenUsage> {
    ...
    if let Some(latest_version) = updates::get_upgrade_version(&config) {
        ...
        {
            let brew_cmd = "brew upgrade icodex";
            lines.push(Line::from(vec![
                "Run ".into(),
                brew_cmd.cyan(),
                " to update.".into(),
    ...
    let auth_manager = AuthManager::shared(config.icodex_home.clone());
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.icodex_home, id_str).await? {
        ...
        match RolloutRecorder::list_conversations(&config.icodex_home, 1, None).await {
            Ok(page) => page
                .items
                .first()
        ...
        match resume_picker::run_resume_picker(&mut tui, &config.icodex_home).await? {
            resume_picker::ResumeSelection::Exit => {
                restore();
                session_log::log_session_end();
                return Ok(icodex_core::protocol::TokenUsage::default());
            }
            other => other,
        }
    } else {
    ...
    if should_show_model_rollout_prompt(
        ...
        internal_storage.gpt_5_icodex_model_prompt_seen,
    ) {
        internal_storage.gpt_5_icodex_model_prompt_seen = true;
...
fn should_show_model_rollout_prompt(
    ...
    gpt_5_icodex_model_prompt_seen: bool,
) -> bool {
    let login_status = get_login_status(config);

    active_profile.is_none()
        && cli.model.is_none()
        && !gpt_5_icodex_model_prompt_seen
...
mod tests {
    use super::*;
    use clap::Parser;
    use icodex_core::auth::AuthDotJson;
    use icodex_core::auth::get_auth_file;
    use icodex_core::auth::login_with_api_key;
    use icodex_core::auth::write_auth_json;
    use icodex_core::token_data::IdTokenInfo;
    use icodex_core::token_data::TokenData;
    fn make_config() -> Config {
        // Create a unique CODEX_HOME per test to isolate auth.json writes.
        let mut icodex_home = std::env::temp_dir();
        ...
        icodex_home.push(unique_suffix);
        std::fs::create_dir_all(&icodex_home).expect("create unique CODEX_HOME");
    ...
    fn set_auth_method(icodex_home: &std::path::Path, mode: AuthMode) {
        match mode {
            AuthMode::ApiKey => {
                login_with_api_key(icodex_home, "sk-test-key").expect("write api key auth.json");
            }
            AuthMode::ChatGPT => {
                ...
                let file = get_auth_file(icodex_home);
    ...
    fn shows_model_rollout_prompt_for_default_model() {
        let cli = Cli::parse_from(["icodex"]);
        let cfg = make_config();
        set_auth_method(&cfg.icodex_home, AuthMode::ChatGPT);
    ...
    fn hides_model_rollout_prompt_when_api_auth_mode() {
        let cli = Cli::parse_from(["icodex"]);
        let cfg = make_config();
        set_auth_method(&cfg.icodex_home, AuthMode::ApiKey);
        assert!(!should_show_model_rollout_prompt(&cli, &cfg, None, false,));
    }
    ...
    fn hides_model_rollout_prompt_when_cli_overrides_model() {
        let cli = Cli::parse_from(["icodex", "--model", "gpt-4.1"]);
        let cfg = make_config();
        set_auth_method(&cfg.icodex_home, AuthMode::ChatGPT);
        assert!(!should_show_model_rollout_prompt(&cli, &cfg, None, false,));
    }
