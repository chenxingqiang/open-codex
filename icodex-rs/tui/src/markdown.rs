...
use icodex_core::config_types::UriBasedFileOpener;
...
mod tests {
    ...
    fn append_markdown_preserves_full_text_line() {
        use icodex_core::config_types::UriBasedFileOpener;
        use std::path::Path;
        let src = "Hi! How can I help with icodex-rs today? Want me to explore the repo, run tests, or work on a specific change?\n";
        let cwd = Path::new("/");
        let mut out = Vec::new();
        append_markdown_with_opener_and_cwd(src, &mut out, UriBasedFileOpener::None, cwd);
        assert_eq!(
        ...
        assert_eq!(
            rendered,
            "Hi! How can I help with icodex-rs today? Want me to explore the repo, run tests, or work on a specific change?"
        );
    }
