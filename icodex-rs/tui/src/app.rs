...
impl App {
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::CodexEvent(event) => {
                self.chat_widget.handle_icodex_event(event);
            }
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                ...
                match persist_model_selection(&self.config.icodex_home, profile, &model, effort)
                    .await
                {
                    Ok(()) => {
    ...
    pub(crate) fn token_usage(&self) -> icodex_core::protocol::TokenUsage {
