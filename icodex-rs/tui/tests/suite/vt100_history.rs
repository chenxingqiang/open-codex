...
struct TestScenario {
    width: u16,
    height: u16,
    term: icodex_tui::custom_terminal::Terminal<TestBackend>,
}

impl TestScenario {
    fn new(width: u16, height: u16, viewport: Rect) -> Self {
        let backend = TestBackend::new(width, height);
        let mut term = icodex_tui::custom_terminal::Terminal::with_options(backend)
    ...
    fn run_insert(&mut self, lines: Vec<Line<'static>>) -> Vec<u8> {
        ...
        icodex_tui::insert_history::insert_history_lines_to_writer(&mut self.term, &mut buf, lines);
        buf
    }
