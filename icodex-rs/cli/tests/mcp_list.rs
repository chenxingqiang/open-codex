...
fn icodex_command(icodex_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::cargo_bin("icodex")?;
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = icodex_command(icodex_home.path())?;
...
fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = icodex_command(icodex_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut list_cmd = icodex_command(icodex_home.path())?;
    ...
    let mut list_json_cmd = icodex_command(icodex_home.path())?;
    ...
    let mut get_cmd = icodex_command(icodex_home.path())?;
    ...
    assert!(stdout.contains("remove: icodex mcp remove docs"));

    let mut get_json_cmd = icodex_command(icodex_home.path())?;
    get_json_cmd
