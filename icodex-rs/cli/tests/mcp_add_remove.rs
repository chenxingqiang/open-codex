...
fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = icodex_command(icodex_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = icodex_command(icodex_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = icodex_command(icodex_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
