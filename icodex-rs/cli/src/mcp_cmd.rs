...
use icodex_core::config::load_global_mcp_servers;
use icodex_core::config::write_global_mcp_servers;
use icodex_core::config_types::McpServerConfig;
...
/// - `add`    — add a server launcher entry to `~/.icodex/config.toml`
...
impl McpCli {
    pub async fn run(self, icodex_linux_sandbox_exe: Option<PathBuf>) -> Result<()> {
        let McpCli {
            config_overrides,
            cmd,
        } = self;
        ...
        match subcommand {
            McpSubcommand::Serve => {
                icodex_mcp_server::run_main(icodex_linux_sandbox_exe, config_overrides).await?;
            }
...
fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let icodex_home = find_icodex_home().context("failed to resolve CODEX_HOME")?;
    let mut servers = load_global_mcp_servers(&icodex_home)
        .with_context(|| format!("failed to load MCP servers from {}", icodex_home.display()))?;
    ...
    write_global_mcp_servers(&icodex_home, &servers)
        .with_context(|| format!("failed to write MCP servers to {}", icodex_home.display()))?;
...
fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `icodex mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: icodex mcp remove {}", get_args.name);
