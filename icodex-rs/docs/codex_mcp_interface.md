...
- Server binary: `icodex mcp` (or `icodex-mcp-server`)
...
- Notifications (server → client)
  ...
  - `icodex/event` stream with agent events
...
- `icodex/event` with the serialized Codex event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
While processing, the server emits `icodex/event` notifications containing agent output, approvals, and status updates.
