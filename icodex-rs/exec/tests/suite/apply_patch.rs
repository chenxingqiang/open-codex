...
/// While we may add an `apply-patch` subcommand to the `icodex` CLI multitool
/// at some point, we must ensure that the smaller `icodex-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("icodex-exec")
        .context("should find binary for icodex-exec")?
