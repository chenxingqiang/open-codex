...
/// Helper function to run an E2E test of a icodex-exec call. Starts a wiremock
/// server, and returns the response_streams in order for each api call. Runs
/// the icodex-exec command with the wiremock server as the model server.
pub(crate) async fn run_e2e_exec_test(cwd: &Path, response_streams: Vec<String>) {
    ...
    Command::cargo_bin("icodex-exec")
        ...
        .expect("should find binary for icodex-exec")
