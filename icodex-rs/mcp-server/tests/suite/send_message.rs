...
use icodex_protocol::mcp_protocol::InputItem;
...
async fn test_send_message_success() {
    ...
    let icodex_home = TempDir::new().expect("create temp dir");
    create_config_toml(icodex_home.path(), &server.uri()).expect("write config.toml");
...
async fn test_send_message_session_not_found() {
    ...
    let icodex_home = TempDir::new().expect("tempdir");
    let mut mcp = McpProcess::new(icodex_home.path()).await.expect("spawn");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
        .expect("timeout")
        .expect("init");
