...
use icodex_login::login_with_api_key;
use icodex_protocol::mcp_protocol::CancelLoginChatGptParams;
use icodex_protocol::mcp_protocol::CancelLoginChatGptResponse;
use icodex_protocol::mcp_protocol::GetAuthStatusParams;
use icodex_protocol::mcp_protocol::GetAuthStatusResponse;
use icodex_protocol::mcp_protocol::LoginChatGptResponse;
use icodex_protocol::mcp_protocol::LogoutChatGptResponse;
...
fn create_config_toml(icodex_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() {
    let icodex_home = TempDir::new().unwrap_or_else(|e| panic!("create tempdir: {e}"));
    create_config_toml(icodex_home.path()).expect("write config.toml");
    login_with_api_key(icodex_home.path(), "sk-test-key").expect("seed api key");
    assert!(icodex_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(icodex_home.path(), &[("OPENAI_API_KEY", None)])
        .await
    ...
    assert!(
        !icodex_home.path().join("auth.json").exists(),
...
async fn login_and_cancel_chatgpt() {
    let icodex_home = TempDir::new().unwrap_or_else(|e| panic!("create tempdir: {e}"));
    create_config_toml(icodex_home.path()).unwrap_or_else(|err| panic!("write config.toml: {err}"));

    let mut mcp = McpProcess::new(icodex_home.path())
        .await
        .expect("spawn mcp process");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
    ...
    let maybe_note = timeout(
        ...
        mcp.read_stream_until_notification_message("icodex/event/login_chat_gpt_complete"),
    )
    .await;
