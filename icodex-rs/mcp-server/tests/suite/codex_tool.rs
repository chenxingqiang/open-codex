...
use icodex_mcp_server::CodexToolCallParam;
use icodex_mcp_server::ExecApprovalElicitRequestParams;
use icodex_mcp_server::ExecApprovalResponse;
use icodex_mcp_server::PatchApprovalElicitRequestParams;
use icodex_mcp_server::PatchApprovalResponse;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "icodex" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let icodex_request_id = mcp_process
        .send_icodex_tool_call(CodexToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        icodex_request_id.to_string(),
        params.icodex_event_id.clone(),
    )?;
    ...
    // Verify the original `icodex` tool call completes and that the file was created.
    let icodex_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(icodex_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(icodex_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    icodex_mcp_tool_call_id: String,
    icodex_event_id: String,
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            icodex_elicitation: "exec-approval".to_string(),
            icodex_mcp_tool_call_id,
            icodex_event_id,
            icodex_command: command,
            icodex_cwd: workdir.to_path_buf(),
            icodex_call_id: "call1234".to_string(),
        })?),
    })
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "icodex" tool request that will trigger the apply_patch command
    let icodex_request_id = mcp_process
        .send_icodex_tool_call(CodexToolCallParam {
    ...
    // Verify the original `icodex` tool call completes
    let icodex_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(icodex_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_icodex_tool_passes_base_instructions() {
    ...
    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = icodex_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn icodex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `icodex mcp` with a specific config.toml.
    let icodex_home = TempDir::new()?;
    create_config_toml(icodex_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(icodex_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request(
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            icodex_elicitation: "patch-approval".to_string(),
            icodex_mcp_tool_call_id,
            icodex_event_id,
            icodex_reason: reason,
            icodex_grant_root: grant_root,
            icodex_changes: changes,
            icodex_call_id: "call1234".to_string(),
        })?),
    })
}
...
fn create_config_toml(icodex_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = icodex_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
