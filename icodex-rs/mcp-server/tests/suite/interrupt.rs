...
async fn shell_command_interruption() -> anyhow::Result<()> {
    ...
    .await;
    create_config_toml(&icodex_home, server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&icodex_home).await?;
...
fn create_config_toml(icodex_home: &Path, server_uri: String) -> std::io::Result<()> {
