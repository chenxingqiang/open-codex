...
use icodex_core::protocol_config_types::ReasoningEffort;
use icodex_core::protocol_config_types::ReasoningSummary;
...
async fn test_icodex_jsonrpc_conversation_flow() {
    ...
    let icodex_home = tmp.path().join("icodex_home");
    std::fs::create_dir(&icodex_home).expect("create icodex home dir");
    ...
    create_config_toml(&icodex_home, &server.uri()).expect("write config");

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&icodex_home).await.expect("spawn mcp");
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![icodex_protocol::mcp_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await
    ...
    let task_finished_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("icodex/event/task_complete"),
    )
    .await
...
async fn test_send_user_turn_changes_approval_policy_behavior() {
    ...
    mcp.send_response(
        request.id,
        serde_json::json!({ "decision": icodex_core::protocol::ReviewDecision::Approved }),
    )
    .await
