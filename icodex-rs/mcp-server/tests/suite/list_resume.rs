...
use icodex_protocol::mcp_protocol::NewConversationParams; // reused for overrides shape
use icodex_protocol::mcp_protocol::ResumeConversationParams;
use icodex_protocol::mcp_protocol::ResumeConversationResponse;
...
async fn test_list_and_resume_conversations() {
    ...
    // Expect a icodex/event notification with msg.type == session_configured
    let notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("icodex/event"),
    )
    .await
...
fn create_fake_rollout(icodex_home: &Path, filename_ts: &str, meta_rfc3339: &str, preview: &str) {
    let uuid = Uuid::new_v4();
    // sessions/YYYY/MM/DD/ derived from filename_ts (YYYY-MM-DDThh-mm-ss)
    let year = &filename_ts[0..4];
    let month = &filename_ts[5..7];
    let day = &filename_ts[8..10];
    let dir = icodex_home.join("sessions").join(year).join(month).join(day);
