...
use icodex_protocol::mcp_protocol::CancelLoginChatGptParams;
use icodex_protocol::mcp_protocol::GetAuthStatusParams;
...
impl McpProcess {
    pub async fn new(icodex_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(icodex_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let std_cmd = StdCommand::cargo_bin("icodex-mcp-server")
            .context("should find binary for icodex-mcp-server")?;
        ...
        cmd.env("CODEX_HOME", icodex_home);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("icodex-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let user_agent = format!(
            "icodex_cli_rs/0.0.0 ({} {}; {}) {} (elicitation test; 0.0.0)",
            os_info.os_type(),
            os_info.version(),
            os_info.architecture().unwrap_or("unknown"),
            icodex_core::terminal::user_agent()
        );
        assert_eq!(
    ...
    pub async fn send_icodex_tool_call(
        &mut self,
        params: CodexToolCallParam,
    ) -> anyhow::Result<i64> {
        let icodex_tool_call_params = CallToolRequestParams {
            name: "icodex".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(icodex_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "icodex/event" with params.msg.type == "task_complete".
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        loop {
            ...
            match message {
                JSONRPCMessage::Notification(notification) => {
                    let is_match = if notification.method == "icodex/event" {
