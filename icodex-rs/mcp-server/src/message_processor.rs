...
use crate::icodex_message_processor::CodexMessageProcessor;
use crate::icodex_tool_config::CodexToolCallParam;
use crate::icodex_tool_config::CodexToolCallReplyParam;
use crate::icodex_tool_config::create_tool_for_icodex_tool_call_param;
use crate::icodex_tool_config::create_tool_for_icodex_tool_call_reply_param;
...
use icodex_core::default_client::USER_AGENT_SUFFIX;
use icodex_core::default_client::get_icodex_user_agent;
use icodex_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    icodex_message_processor: CodexMessageProcessor,
    outgoing: Arc<OutgoingMessageSender>,
    initialized: bool,
    icodex_linux_sandbox_exe: Option<PathBuf>,
    conversation_manager: Arc<ConversationManager>,
    running_requests_id_to_icodex_uuid: Arc<Mutex<HashMap<RequestId, ConversationId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        let icodex_message_processor = CodexMessageProcessor::new(
            auth_manager,
            conversation_manager.clone(),
            outgoing.clone(),
            icodex_linux_sandbox_exe.clone(),
            config,
        );
        Self {
            icodex_message_processor,
            outgoing,
            initialized: false,
            icodex_linux_sandbox_exe,
            conversation_manager,
            running_requests_id_to_icodex_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    pub(crate) async fn process_request(&mut self, request: JSONRPCRequest) {
        if let Ok(request_json) = serde_json::to_value(request.clone())
            && let Ok(icodex_request) = serde_json::from_value::<ClientRequest>(request_json)
        {
            // If the request is a Codex request, handle it with the Codex
            // message processor.
            self.icodex_message_processor
                .process_request(icodex_request)
                .await;
            return;
        }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "icodex-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("Codex".to_string()),
                user_agent: Some(get_icodex_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_icodex_tool_call_param(),
                create_tool_for_icodex_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "icodex" => self.handle_tool_call_icodex(id, arguments).await,
            "icodex-reply" => {
                self.handle_tool_call_icodex_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_icodex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.icodex_linux_sandbox_exe.clone()) {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for icodex tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        let running_requests_id_to_icodex_uuid = self.running_requests_id_to_icodex_uuid.clone();

        // Spawn an async task to handle the Codex session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Codex session and stream events back to the client.
            crate::icodex_tool_runner::run_icodex_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_icodex_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_icodex_session_reply(
        &self,
        ...
        let CodexToolCallReplyParam {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for icodex-reply tool-call; the `conversation_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for icodex-reply tool-call; the `conversation_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_icodex_uuid = running_requests_id_to_icodex_uuid.clone();

            async move {
                crate::icodex_tool_runner::run_icodex_tool_session_reply(
                    icodex,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let conversation_id = {
            let map_guard = self.running_requests_id_to_icodex_uuid.lock().await;
        ...
        let err = icodex_arc
            .submit_with_id(Submission {
                ...
                op: icodex_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
        ...
        self.running_requests_id_to_icodex_uuid
            .lock()
            .await
            .remove(&request_id);
    }
