//! Configuration object accepted by the `icodex` MCP tool-call.
...
/// Client-supplied configuration for a `icodex` tool-call.
...
pub(crate) fn create_tool_for_icodex_tool_call_param() -> Tool {
...
impl CodexToolCallParam {
    ...
    pub fn into_config(
    ...
    ) -> std::io::Result<(String, icodex_core::config::Config)> {
        let Self {
            prompt,
            model,
            profile,
        ...
        // Build the `ConfigOverrides` recognized by icodex-core.
        let overrides = icodex_core::config::ConfigOverrides {
            model,
            review_model: None,
        ...
        let cfg = icodex_core::config::Config::load_with_cli_overrides(cli_overrides, overrides)?;
...
/// Builds a `Tool` definition for the `icodex-reply` tool-call.
pub(crate) fn create_tool_for_icodex_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
...
mod tests {
    ...
    #[test]
    fn verify_icodex_tool_json_schema() {
        let tool = create_tool_for_icodex_tool_call_param();
    ...
    #[test]
    fn verify_icodex_tool_reply_json_schema() {
        let tool = create_tool_for_icodex_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
