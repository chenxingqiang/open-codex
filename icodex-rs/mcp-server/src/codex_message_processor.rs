...
use icodex_core::AuthManager;
use icodex_core::CodexConversation;
use icodex_core::ConversationManager;
use icodex_core::Cursor as RolloutCursor;
use icodex_core::NewConversation;
use icodex_core::RolloutRecorder;
use icodex_core::SessionMeta;
use icodex_core::auth::CLIENT_ID;
use icodex_core::auth::get_auth_file;
use icodex_core::auth::login_with_api_key;
use icodex_core::auth::try_read_auth_json;
use icodex_core::config::Config;
use icodex_core::config::ConfigOverrides;
use icodex_core::config::ConfigToml;
use icodex_core::config::load_config_as_toml;
use icodex_core::config_edit::CONFIG_KEY_EFFORT;
use icodex_core::config_edit::CONFIG_KEY_MODEL;
use icodex_core::config_edit::persist_overrides_and_clear_if_none;
use icodex_core::default_client::get_icodex_user_agent;
use icodex_core::exec::ExecParams;
use icodex_core::exec_env::create_env;
use icodex_core::get_platform_sandbox;
use icodex_core::git_info::git_diff_to_remote;
use icodex_core::protocol::ApplyPatchApprovalRequestEvent;
use icodex_core::protocol::Event;
use icodex_core::protocol::EventMsg;
use icodex_core::protocol::ExecApprovalRequestEvent;
use icodex_core::protocol::InputItem as CoreInputItem;
use icodex_core::protocol::Op;
use icodex_core::protocol::ReviewDecision;
use icodex_login::ServerOptions as LoginServerOptions;
use icodex_login::ShutdownHandle;
use icodex_login::run_login_server;
use icodex_protocol::mcp_protocol::APPLY_PATCH_APPROVAL_METHOD;
use icodex_protocol::mcp_protocol::AddConversationListenerParams;
use icodex_protocol::mcp_protocol::AddConversationSubscriptionResponse;
use icodex_protocol::mcp_protocol::ApplyPatchApprovalParams;
use icodex_protocol::mcp_protocol::ApplyPatchApprovalResponse;
use icodex_protocol::mcp_protocol::ArchiveConversationParams;
use icodex_protocol::mcp_protocol::ArchiveConversationResponse;
use icodex_protocol::mcp_protocol::AuthStatusChangeNotification;
use icodex_protocol::mcp_protocol::ClientRequest;
use icodex_protocol::mcp_protocol::ConversationId;
use icodex_protocol::mcp_protocol::ConversationSummary;
use icodex_protocol::mcp_protocol::EXEC_COMMAND_APPROVAL_METHOD;
use icodex_protocol::mcp_protocol::ExecArbitraryCommandResponse;
use icodex_protocol::mcp_protocol::ExecCommandApprovalParams;
use icodex_protocol::mcp_protocol::ExecCommandApprovalResponse;
use icodex_protocol::mcp_protocol::ExecOneOffCommandParams;
use icodex_protocol::mcp_protocol::GetUserAgentResponse;
use icodex_protocol::mcp_protocol::GetUserSavedConfigResponse;
use icodex_protocol::mcp_protocol::GitDiffToRemoteResponse;
use icodex_protocol::mcp_protocol::InputItem as WireInputItem;
use icodex_protocol::mcp_protocol::InterruptConversationParams;
use icodex_protocol::mcp_protocol::InterruptConversationResponse;
use icodex_protocol::mcp_protocol::ListConversationsParams;
use icodex_protocol::mcp_protocol::ListConversationsResponse;
use icodex_protocol::mcp_protocol::LoginApiKeyParams;
use icodex_protocol::mcp_protocol::LoginApiKeyResponse;
use icodex_protocol::mcp_protocol::LoginChatGptCompleteNotification;
use icodex_protocol::mcp_protocol::LoginChatGptResponse;
use icodex_protocol::mcp_protocol::NewConversationParams;
use icodex_protocol::mcp_protocol::NewConversationResponse;
use icodex_protocol::mcp_protocol::RemoveConversationListenerParams;
use icodex_protocol::mcp_protocol::RemoveConversationSubscriptionResponse;
use icodex_protocol::mcp_protocol::ResumeConversationParams;
use icodex_protocol::mcp_protocol::SendUserMessageParams;
use icodex_protocol::mcp_protocol::SendUserMessageResponse;
use icodex_protocol::mcp_protocol::SendUserTurnParams;
use icodex_protocol::mcp_protocol::SendUserTurnResponse;
use icodex_protocol::mcp_protocol::ServerNotification;
use icodex_protocol::mcp_protocol::SetDefaultModelParams;
use icodex_protocol::mcp_protocol::SetDefaultModelResponse;
use icodex_protocol::mcp_protocol::UserInfoResponse;
use icodex_protocol::mcp_protocol::UserSavedConfig;
use icodex_protocol::models::ContentItem;
use icodex_protocol::models::ResponseItem;
use icodex_protocol::protocol::InputMessageKind;
use icodex_protocol::protocol::USER_MESSAGE_BEGIN;
...
pub(crate) struct CodexMessageProcessor {
    ...
    icodex_linux_sandbox_exe: Option<PathBuf>,
    config: Arc<Config>,
...
impl CodexMessageProcessor {
    ...
    async fn login_api_key(&mut self, request_id: RequestId, params: LoginApiKeyParams) {
        ...
        match login_with_api_key(&self.config.icodex_home, &params.api_key) {
            Ok(()) => {
    ...
    async fn login_chatgpt(&mut self, request_id: RequestId) {
        ...
        let opts = LoginServerOptions {
            open_browser: false,
            ..LoginServerOptions::new(config.icodex_home.clone(), CLIENT_ID.to_string())
        };
    ...
    async fn cancel_login_chatgpt(&mut self, request_id: RequestId, login_id: Uuid) {
        ...
        if guard.as_ref().map(|l| l.login_id) == Some(login_id) {
            ...
            self.outgoing
                .send_response(
                    request_id,
                    icodex_protocol::mcp_protocol::CancelLoginChatGptResponse {},
                )
                .await;
        } else {
            drop(guard);
    ...
    async fn logout_chatgpt(&mut self, request_id: RequestId) {
        ...
        self.outgoing
            .send_response(
                request_id,
                icodex_protocol::mcp_protocol::LogoutChatGptResponse {},
            )
            .await;
    ...
    async fn get_auth_status(
        ...
        params: icodex_protocol::mcp_protocol::GetAuthStatusParams,
    ) {
        ...
        let response = if !requires_openai_auth {
            icodex_protocol::mcp_protocol::GetAuthStatusResponse {
                auth_method: None,
                auth_token: None,
            ...
            match self.auth_manager.auth() {
                ...
                None => icodex_protocol::mcp_protocol::GetAuthStatusResponse {
                    auth_method: None,
                    auth_token: None,
                    requires_openai_auth: Some(true),
                },
    ...
    async fn get_user_agent(&self, request_id: RequestId) {
        let user_agent = get_icodex_user_agent();
        let response = GetUserAgentResponse { user_agent };
        self.outgoing.send_response(request_id, response).await;
    }

    async fn get_user_saved_config(&self, request_id: RequestId) {
        let toml_value = match load_config_as_toml(&self.config.icodex_home) {
            Ok(val) => val,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
    ...
    async fn get_user_info(&self, request_id: RequestId) {
        ...
        let auth_path = get_auth_file(&self.config.icodex_home);
    ...
    async fn exec_one_off_command(&self, request_id: RequestId, params: ExecOneOffCommandParams) {
        ...
        let sandbox_type = match &effective_policy {
            icodex_core::protocol::SandboxPolicy::DangerFullAccess => {
                icodex_core::exec::SandboxType::None
            }
            _ => get_platform_sandbox().unwrap_or(icodex_core::exec::SandboxType::None),
        };
        tracing::debug!("Sandbox type: {sandbox_type:?}");
        let icodex_linux_sandbox_exe = self.config.icodex_linux_sandbox_exe.clone();
        let outgoing = self.outgoing.clone();
        let req_id = request_id;

        tokio::spawn(async move {
            match icodex_core::exec::process_exec_tool_call(
                exec_params,
                sandbox_type,
                &effective_policy,
                &icodex_linux_sandbox_exe,
    ...
    async fn process_new_conversation(&self, request_id: RequestId, params: NewConversationParams) {
        let config = match derive_config_from_params(params, self.icodex_linux_sandbox_exe.clone()) {
            Ok(config) => config,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
    ...
    async fn handle_resume_conversation(
        ...
        let config = match params.overrides {
            Some(overrides) => {
                derive_config_from_params(overrides, self.icodex_linux_sandbox_exe.clone())
            }
            ...
            .await
        {
            Ok(NewConversation {
                ...
                let response = icodex_protocol::mcp_protocol::ResumeConversationResponse {
                    conversation_id,
    ...
    async fn archive_conversation(&self, request_id: RequestId, params: ArchiveConversationParams) {
        ...
        let rollout_folder = self.config.icodex_home.join(icodex_core::SESSIONS_SUBDIR);
        ...
        let result: std::io::Result<()> = async {
            let archive_folder = self
                .config
                .icodex_home
                .join(icodex_core::ARCHIVED_SESSIONS_SUBDIR);
            ...
            loop {
                tokio::select! {
                    ...
                    event = conversation.next_event() => {
                        ...
                        // instead.
                        let method = format!("icodex/event/{}", event.msg);
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `icodex`.
    let response =
